...
class MigrateCommitNotesMentionsToDb < ActiveRecord::Migration[5.2]
  ...
  INDEX_CONDITION = "note LIKE '%@%'::text AND notes.noteable_type = 'Commit'"
  QUERY_CONDITIONS = "#{INDEX_CONDITION} AND commit_user_mentions.commit_id IS NULL"
  ...
  def up
    ...
    Note
      ...
      BackgroundMigrationWorker.perform_in(index * DELAY, MIGRATION, ['Commit', JOIN, QUERY_CONDITIONS, true, *range])
    end
  end
