...
import { issuableTypesMap, linkedIssueTypesMap, PathIdSeparator } from 'ee/related_issues/constants';
  ...
  describe('with data', () => {
    ...
    describe('when issuable type is "issue"', () => {
      ...
      it('shows radio inputs to allow categorisation of blocking issues', () => {
        expect(findRadioInputs.length).toBeGreaterThan(0);
      });
    ...
    describe('when issuable type is "epic"', () => {
      ...
      it('does not show radio inputs', () => {
        expect(findRadioInputs.length).toBe(0);
      });
    });
  });
});
