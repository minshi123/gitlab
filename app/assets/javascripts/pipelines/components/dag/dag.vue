  ...
  mounted() {
    ...
    axios.get(this.graphUrl)
      .then(function (response) {
        ...
        // drawGraph(simplified, parseData);
        // drawGraph(stages, parseData);
        // drawGraph(nestedStages, parseNestedData);
        drawGraph(endpointStages, parseNestedData);
      })
...
<template>
  <div>
    <div v-if='!this.showEmptyState' class='dag-graph-container gl-pt-9'></div>
    <div class='gl-pt-9' v-else>
      {{ __('This pipeline does not use the needs keyword and therefore does not have a directed acyclic graph.') }}
    </div>
  </div>
</template>
