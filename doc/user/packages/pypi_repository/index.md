...
After creating this, create another directory inside, like so:

```shell
mkdir mypypipackage && cd mypypipackage
```

Create two new files inside this directory to set up the basic package:
...
Now that the basics of our package is completed, let's test that the code runs.
...
Inside your `MyPyPiPackage` directory, run the following:

```shell
touch setup.py
```

Next, add the below content to `setup.py`:
