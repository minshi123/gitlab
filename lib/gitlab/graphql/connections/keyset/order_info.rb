      ...
      module Keyset
        class OrderInfo
          ...
          def extract_attribute_values(order_value)
            name = if order_value.expr.is_a?(Arel::Nodes::NamedFunction)
                     # for example, "ORDER BY LOWER(boards.name)" should
                     # return "LOWER(boards.name)"
                     order_value.expr.to_sql
                   else
                     order_value.expr.name
                   end

            [name, order_value.direction]
          end
        end
      end
    end
