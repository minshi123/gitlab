...
class Packages::Package < ApplicationRecord
  ...
  scope :order_project_path, -> { joins(:project).reorder('projects.path ASC, created_at ASC') }
  scope :order_project_path_desc, -> { joins(:project).reorder('projects.path DESC, created_at DESC') }
