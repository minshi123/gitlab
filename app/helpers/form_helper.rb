...
module FormHelper
  ...
  def reviewers_dropdown_options(issuable_type)
    ...
    if type == 'issue' && issue_supports_multiple_reviewers? ||
        type == 'merge_request' && merge_request_supports_multiple_reviewers?
  ...
  # Overwritten
  def issue_supports_multiple_reviewers?
    false
  end
  ...
  def multiple_reviewers_dropdown_options(options)
    ...
    new_options[:title] = 'Select reviewer(s) 123'
    new_options[:data][:'dropdown-header'] = 'Reviewer(s) 456'
    new_options[:data].delete(:'max-select')
