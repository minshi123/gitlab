...
export const getFormatter = (format = 'number') => {
  ...
  if (format === SUPPORTED_FORMATS.percent) {
     ...
     * if lenght is exceeded, exponential format is used.
     */
    return suffixFormatter(s__('Units|%'));
  }
  if (format === SUPPORTED_FORMATS.percentunit) {
    /**
     * Formats a percentge (0 to 1)
     *
     * @function
     ...
     * if lenght is exceeded, exponential format is used.
     */
    return suffixFormatter(s__('Units|%'), 100);
  }
