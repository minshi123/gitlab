  ...
  describe '#execute' do
    ...
    context 'with an existing package' do
      ...
      context 'with an existing file' do
        before do
          params[:file] = temp_file('foo.tgz')
          params[:sha256_digest] = 'abc'
          params[:md5_digest] = 'def'
        end

        it 'replaces the file' do
          expect { subject }.to change { Packages::Package.pypi.count }.by(0).and change { Packages::PackageFile.count }.by(0)
      ...
      context 'without an existing file' do
        before do
          params[:file] = temp_file('another.tgz')
        end

        it 'adds the file' do
          expect { subject }.to change { Packages::Package.pypi.count }.by(0).and change { Packages::PackageFile.count }.by(1)
