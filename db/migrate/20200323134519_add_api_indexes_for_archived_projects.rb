...
class AddApiIndexesForArchivedProjects < ActiveRecord::Migration[6.0]
  ...
  def up
    add_concurrent_index :projects, [:created_at, :id], where: "archived = true AND visibility_level = 20",
      name: PUBLIC_AND_ARCHIVED_INDEX_NAME

    add_concurrent_index :projects, [:created_at, :id], where: "archived = true", name: ARCHIVED_INDEX_NAME
  end
