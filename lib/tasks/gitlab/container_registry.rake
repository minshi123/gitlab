namespace :gitlab do
  namespace :container_registry do
    ...
    task configure: :gitlab_environment do
      ...
      # requires authentication, but not authorization. There is no associated
      # user when running this rake, so we need to generate a valid JWT token
      # with no access permissions to authenticate as a trusted client.
