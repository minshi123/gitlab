...
module TabHelper
  ...
  #           :skip_if      - Callable object to skip rendering the 'active' class on `li` element (optional).
  ...
  #   # => '<li class="active">Hello</li>'
  #
  #   # Shorthand path + skip_if
  #   # Add `active` class when TreeController is requested, except the `index` action.
  #   nav_link(controller: 'tree', skip_if: -> { action_name?('index') }) { "Hello" }
  #   # => '<li class="active">Hello</li>'
  #
  ...
  def active_nav_link?(options)
    return false if options[:skip_if]&.call
