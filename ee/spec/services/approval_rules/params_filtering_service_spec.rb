      ...
      context 'inapplicable user defined rules' do
        ...
        context 'when multiple_approval_rules feature is available' do
          ...
          it 'adds inapplicable user defined rules' do
            ...
            expect(params[:approval_rules_attributes].size).to eq(2)

            expect(params[:approval_rules_attributes].first[:name]).to eq(another_source_rule.name)
            expect(params[:approval_rules_attributes].first[:approval_project_rule_id]).to eq(another_source_rule.id)

            expect(params[:approval_rules_attributes].last[:name]).to eq(source_rule.name)
            expect(params[:approval_rules_attributes].last[:approval_project_rule_id]).to eq(source_rule.id)
            expect(params[:approval_rules_attributes].last[:user_ids]).to eq(source_rule.user_ids)
            expect(params[:approval_rules_attributes].last[:group_ids]).to eq(source_rule.group_ids)
            expect(params[:approval_rules_attributes].last[:approvals_required]).to eq(source_rule.approvals_required)
            expect(params[:approval_rules_attributes].last[:rule_type]).to eq(source_rule.rule_type)
          end
