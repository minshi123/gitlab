...
export default {
  ...
  computed: {
    assignedUsers() {
      return this.alert.assignees.length > 0
        ? this.alert.assignees[0].username
  ...
  methods: {
    ...
    isActive(name) {
      return this.alert.assignees.some(({ username }) => username === name);
    },
  ...
  <div class="block alert-status">
    ...
    <div class="hide-collapsed">
      ...
      <p
        ...
        class="value gl-m-0"
        :class="{ 'no-value': !alert.assignees }"
      >
        <span v-if="alert.assignees" class="gl-text-gray-700" data-testid="assigned-users">{{
          assignedUsers
        }}</span>
        <span v-else>
