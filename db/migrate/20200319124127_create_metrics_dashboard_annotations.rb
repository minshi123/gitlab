...
class CreateMetricsDashboardAnnotations < ActiveRecord::Migration[6.0]
  ...
  def change
    create_table :metrics_dashboard_annotations do |t|
      ...
      t.index %i(environment_id dashboard_path starting_at ending_at panel_xid), where: 'environment_id IS NOT NULL', name: "index_metrics_dashboard_annotations_on_environment_id_and_4_col"
      t.index %i(cluster_id dashboard_path starting_at ending_at panel_xid), where: 'cluster_id IS NOT NULL', name: "index_metrics_dashboard_annotations_on_cluster_id_and_4_columns"
    end
  end
end
