...
describe MergeRequests::SquashService do
  ...
  describe '#execute' do
    ...
    context 'when squashing is disabled by default on the project' do
      ...
      # to squash if selected through the UI
      let(:merge_request) { merge_request_with_only_new_files }

      before do
        merge_request.project.squash_option = :squash_disabled
      end
    ...
    context 'when squashing is forbidden on the project' do
      ...
      before do
        merge_request.project.squash_option = :never_squash
      end

      it 'raises a squash error' do
        expect(service.execute).to match(
          status: :error,
          message: a_string_including('prohibits squashing'))
      end
    end

    context 'when squashing is enabled on the project' do
      let(:merge_request) { merge_request_with_only_new_files }

      before do
        merge_request.project.squash_option = :squash_enabled
      end

      include_examples 'the squash succeeds'
    end

    context 'when squashing is required on the project' do
      let(:merge_request) { merge_request_with_only_new_files }

      before do
        merge_request.project.squash_option = :always_squash
      end
