      ...
      return this.$apollo
        ...
        .then(({ data }) => {
          if (!data.updateRequirement.errors.length) {
            ...
            // TODO: Find a way to get VueApollo autosync after mutation
            // instead of manually updating the data prop.
            const updatedRequirement = data.updateRequirement.requirement;
            const staleRequirement = this.requirements.list.filter(
              req => req.iid === updatedRequirement.iid,
            );
            Object.assign(staleRequirement[0], updatedRequirement);
          } else {
