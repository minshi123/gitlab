...
import { timestampToISODate, formatDate, masks } from '../../dateformat';
...
export default {
  ...
  computed: {
    ...
    chartOptions() {
      ...
      const timeXAxis = {
        ...
        type: 'time',
        axisLabel: {
          formatter: date => formatDate(date, masks.isoTime),
        },
        axisPointer: {
          snap: true,
        },
  ...
  methods: {
    ...
    formatAnnotationsTooltipText(params) {
      return {
        title: formatDate(params.data?.tooltipData?.title),
        content: params.data?.tooltipData?.content,
      };
    },
    formatTooltipText(params) {
      this.tooltip.title = formatDate(params.value);
