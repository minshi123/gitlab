...
describe Epics::DescendantCountService do
  ...
  let_it_be(:issue3) { create(:issue, project: project, state: :opened, health_status: :needs_attention) }
  let_it_be(:issue4) { create(:issue, project: project, state: :closed, health_status: :at_risk) }
  ...
  describe '#issues_needs_attention' do
    it_behaves_like 'descendants state count', :issues_needs_attention, 1
  end
