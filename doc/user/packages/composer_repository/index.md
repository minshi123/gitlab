...
> [Introduced](https://gitlab.com/gitlab-org/gitlab/-/issues/15886) in [GitLab Premium](https://about.gitlab.com/pricing/) 13.10.

With the GitLab Composer Repository, every project can have its own space to store Composer packages.

The GitLab Composer Repository works with:
...
(Make sure a GitLab repository is created before hand to host the package).
...
```shell
curl -v -d tag=<tag> 'https://__token__:<personal-access-token>@gitlab.com/api/v4/projects/<project_id>/packages/composer'
```
...
Add the the GitLab Composer package repository to your existing project's `composer.json` file, along with the package name and version you want to install like so:
