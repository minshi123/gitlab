...
describe Gitlab::Elastic::Indexer do
  ...
  context 'empty project' do
    ...
    context 'when indexing an unborn head' do
      it 'updates the index status without running the indexing command' do
  ...
  context 'with an indexed project' do
    ...
    context 'when indexing a HEAD commit' do
    ...
    context 'when indexing a non-HEAD commit' do
      let(:to_sha) { project.repository.commit('HEAD~1').sha }

      it_behaves_like 'index up to the specified commit'

      context 'after reverting a change', :elastic do
    ...
    context "when indexing a project's wiki" do
