...
describe Packages::Composer::CreatePackageService do
  ...
  let_it_be(:json) { { name: 'composer-package-name' }.to_json }
  ...
  describe '#execute' do
    ...
    context 'without an existing package' do
      context 'with a branch' do
        ...
        it 'creates the package' do
          expect { subject }.to change { Packages::Package.composer.count }.by(1)

          expect(created_package.name).to eq 'composer-package-name'
    ...
    context 'with an existing package' do
      ...
      context 'belonging to another project' do
        let(:other_project) { create(:project, :custom_repo, files: { 'composer.json' => json } ) }

        before do
          described_class.new(other_project, user, { branch: other_project.repository.find_branch('master') }).execute
        end
