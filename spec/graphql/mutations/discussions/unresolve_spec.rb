...
describe Mutations::Discussions::Unresolve do
  subject(:mutation) do
  ...
  describe '#resolve' do
    ...
    shared_examples 'a working resolve method' do
      ...
      context 'when the user has permission' do
        ...
        context 'when discussion is not a Discussion' do
          let(:discussion) { create(:note, :resolved, noteable: noteable, project: project) }
        ...
        context 'when a `RecordNotSaved` error is encountered when unresolving' do
          before do
            allow_next_instance_of(discussion.class) do |instance|
              allow(instance).to receive(:unresolve!).and_raise(ActiveRecord::RecordNotSaved, 'test error')
            end
          end
    ...
    context 'when discussion is on a merge request' do
      ...
      let(:discussion) { create(:diff_note_on_merge_request, :resolved, noteable: noteable, project: project).to_discussion }
    ...
    context 'when discussion is on a design' do
      ...
      let(:discussion) { create(:diff_note_on_design, :resolved, noteable: noteable, project: project).to_discussion }
