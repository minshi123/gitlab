...
module ApplicationSettingImplementation
  ...
  def issues_create_limit_users_whitelist_raw
    array_to_string(self.issues_create_limit_users_whitelist)
  end

  def issues_create_limit_users_whitelist_raw=(values)
    clear_memoization(:issues_create_limit_users_whitelist)

    self.issues_create_limit_users_whitelist = strings_to_array(values)
  end
