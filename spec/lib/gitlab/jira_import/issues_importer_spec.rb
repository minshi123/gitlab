...
describe Gitlab::JiraImport::IssuesImporter do
  ...
  describe '#execute', :clean_gitlab_redis_cache do
    ...
    context 'with results returned' do
      ...
      def mock_issue_serializer(count = 2)
        serializer = double(execute: { key: 'data' })
      ...
      context 'when single page of results is returned' do
        ...
        it 'schedules 2 import jobs' do
          expect(subject).to receive(:fetch_issues).and_return([jira_issues[0], jira_issues[1]])
      ...
      context 'when duplicate results are returned' do
        ...
        it 'schedules 2 import jobs' do
          expect(subject).to receive(:fetch_issues).and_return([jira_issues[0], jira_issues[0]])
