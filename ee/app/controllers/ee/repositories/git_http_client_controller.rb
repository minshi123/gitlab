...
module EE
  module Repositories
    module GitHttpClientController
      ...
      #
      # * git pull (project not yet replicated)
      #   * GET   /namespace/repo.git/info/refs?service=git-upload-pack
      #
      # * git lfs pull (project not yet replicated)
      ...
      class GeoRouteHelper
        ...
        def redirect?
          !!CONTROLLER_AND_ACTIONS_TO_REDIRECT[controller_name]&.include?(action_name) ||
            ...
            geo_not_yet_replicated_redirect?
        end

        def geo_not_yet_replicated_redirect?
          return unless project

          git_upload_pack_request? && !project.repository.exists?
        end
      ...
      class GeoGitLFSHelper
        ...
        def redirect?
          ...
          return true if geo_not_yet_replicated_redirect?
        ...
        def geo_not_yet_replicated_redirect?
          ...
          (batch_download? || transfer_download?) && !project.repository.exists?
        end
      ...
      def geo_primary_full_url
        path = if geo_route_helper.geo_not_yet_replicated_redirect?
                 # git clone/pull
