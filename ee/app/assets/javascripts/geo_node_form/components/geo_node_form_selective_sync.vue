<script>
import { GlFormGroup, GlFormSelect, GlFormCheckbox, GlSprintf } from '@gitlab/ui';
...
<template>
  <div ref="geoNodeFormSelectiveSyncContainer">
    ...
    <p class="gl-mb-5">
      ...
      <a :href="$options.SELECTIVE_SYNC_MORE_INFO" target="_blank">{{ __('More information') }}</a>
    </p>
    <gl-form-group
    ...
    <gl-form-group :label="__('Object Storage replication')" label-for="node-object-storage-field">
      <template #description>
        <gl-sprintf
          :message="
            __(
              'If enabled, GitLab will handle Object Storage replication using Geo %{moreInformation}',
            )
          "
        >
          <template #moreInformation>
            <a :href="$options.OBJECT_STORAGE_MORE_INFO" target="_blank">{{
              __('More information')
            }}</a>
          </template>
        </gl-sprintf>
