...
describe('isValidDate', () => {
  it.each`
    valueToCheck                              | expectedReturnValue
    ${new Date()}                             | ${true}
    ...
    ${null}                                   | ${false}
  `(
    'returns $expectedReturnValue when called with $dateToCheck',
    ({ valueToCheck, expectedReturnValue }) => {
      expect(datetimeUtility.isValidDate(valueToCheck)).toBe(expectedReturnValue);
    },
  );
});
