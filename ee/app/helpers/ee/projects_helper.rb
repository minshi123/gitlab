...
module EE
  module ProjectsHelper
    ...
    # references after we remove the old findings dashboard.
    def security_dashboard_pipeline_data(project, pipeline)
      {
        pipeline_id: pipeline&.id,
        user_path: pipeline && user_url(pipeline.user),
        user_avatar_path: pipeline&.user&.avatar_url,
        user_name: pipeline&.user&.name,
        commit_id: pipeline&.commit&.short_id,
        commit_path: pipeline && project_commit_url(project, pipeline.commit),
        ref_id: pipeline&.ref,
        ref_path: pipeline && project_commits_url(project, pipeline.ref),
        pipeline_path: pipeline && pipeline_url(pipeline),
        pipeline_created: pipeline&.created_at&.to_s(:iso8601),
        has_pipeline_data: pipeline.present?
      }
    end
