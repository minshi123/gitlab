...
class MigrateIssueNotesMentionsToDb < ActiveRecord::Migration[5.2]
  ...
  INDEX_CONDITION = "note LIKE '%@%'::text AND notes.noteable_type = 'Issue'"
  QUERY_CONDITIONS = "#{INDEX_CONDITION} AND issue_user_mentions.issue_id IS NULL"
  JOIN = 'LEFT JOIN issue_user_mentions ON notes.id = issue_user_mentions.note_id'
  ...
  def up
    ...
    Note
      ...
      BackgroundMigrationWorker.perform_in(index * DELAY, MIGRATION, ['Issue', JOIN, QUERY_CONDITIONS, true, *range])
    end
  end
