...
describe Gitlab::BackgroundMigration::UserMentions::CreateResourceUserMention do
  ...
  context 'migrate issue mentions' do
    ...
    let!(:issue) { issues.create!(project_id: project.id, author_id: author.id, description: description_mentions) }
    let!(:issue_without_mentions) { issues.create!(project_id: project.id, author_id: author.id, description: 'some description') }
