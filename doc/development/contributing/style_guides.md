...
You're strongly advised to install [Overcommit](https://github.com/sds/overcommit)
to automatically check for static analysis offenses before committing locally:

```shell
mv .overcommit.yml.example .overcommit.yml

bundle install --gemfile=tooling/overcommit/gems.rb
...
Similarly, some RuboCop rules are currently explicitely disabled, and for those,
reviewers/maintainers must not ask author to use one style or the other, as both
are accepted. This isn't an ideal situation since this leaves space for
bike-shedding, and ideally we should enable all RuboCop rules to avoid
style-related discussions/nitpicking/back-and-forth in reviews.

Additionaly, we have a dedicated [newlines styleguide](../newlines_styleguide.md),
