  ...
  module Elastic
    class Indexer
      ...
      def build_aws_credentials_env(vars)
        ...
        %w(access_key_id secret_access_key session_token).each_with_object(vars) do |key, hash|
          value = credentials.public_send(key) # rubocop:disable GitlabSecurity/PublicSend
          hash['AWS_' + key.upcase] = value
        end
      end
