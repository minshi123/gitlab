...
```mermaid
graph TD
    build-qa-image -->|once the `prepare` stage is done| gitlab:assets:compile pull-cache
    gitlab:assets:compile pull-cache -->|once the `gitlab:assets:compile` job is done| review-build-cng
