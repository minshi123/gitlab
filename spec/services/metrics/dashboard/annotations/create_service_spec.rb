...
describe Metrics::Dashboard::Annotations::CreateService do
  ...
  let(:dashboard_id) { 'config/prometheus/common_metrics.yml' }
  let(:from) { 15.minutes.ago }
  let(:to) { nil }
  ...
  describe '.execute' do
    ...
    context 'missing dashboard_id' do
      let(:cluster) { create(:cluster, :project) }
      let(:environment) { nil }
    ...
    context 'incorrect dashboard_id' do
      let(:cluster) { create(:cluster, :project) }
      let(:environment) { nil }
      let(:dashboard_id) { 'something_incorrect.yml' }
      ...
      context 'with maintainer user' do
        ...
        it_behaves_like 'prevented annotation creation', 'Dashboard with requested id can not be found'
      end
    end
  end
end
