...
import FirstClassGroupVulnerabilities from 'ee/security_dashboard/components/first_class_group_dashboard_vulnerabilities.vue';
...
describe('First Class Group Dashboard Vulnerabilities Component', () => {
  ...
  const emptyStateText =
    "While it's rare to have no vulnerabilities for your group, it can happen. " +
    "In any event, we ask that you double check your settings to make sure you've set up your dashboards correctly.";
  ...
  describe('when the query returned an empty vulnerability list', () => {
    ...
    it('displays the empty state', () => {
      expect(findEmptyState().text()).toContain(emptyStateText);
    });
  });
  ...
  describe('when the query is loaded and we have results', () => {
    ...
    it('does not have an empty state', () => {
      expect(findEmptyState().text()).not.toContain(emptyStateText);
    });
