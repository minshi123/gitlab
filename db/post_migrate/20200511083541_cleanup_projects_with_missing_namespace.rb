...
class CleanupProjectsWithMissingNamespace < ActiveRecord::Migration[6.0]
    ...
    class << self
      ...
      def create_unique_internal(scope, username, email_pattern, &creation_block)
        ...
        User.create!(
          ...
          projects_limit: Gitlab::CurrentSettings.default_projects_limit,
          state: :active,
          &creation_block
        )
      ensure
  ...
  class Group < Namespace
    ...
    def self.create_unique_group(user, group_name)
      ...
      group_name = Uniquify.new.string(group_name) do |str|
        Group.where(parent_id: nil).find_by(name: str).present?
      end
  ...
  def up
    ...
    Project.without_namespace.each_batch(of: BATCH_SIZE) do |relation|
