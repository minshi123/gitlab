...
class IssuableFinder
  ...
  NEGATABLE_PARAMS_HELPER_KEYS = %i[include_subgroups].freeze
  ...
  class << self
    ...
    def negatable_scalar_params
      @negatable_scalar_params ||= scalar_params + %i[project_id group_id] - %i[search in]
    end
  ...
  def initialize(current_user, params = {})
    ...
    @params = Params.new(params, current_user, klass)
  end

  def not_params
    strong_memoize(:not_params) do
      Params.new(params[:not].dup, current_user, klass).tap do |not_params|
        if not_params.present?
  ...
  def filter_negated_items(items)
    ...
    items = by_negated_project(items)
  ...
  # rubocop: disable CodeReuse/ActiveRecord
  def by_negated_project(items)
    if not_params.project? || (not_params.projects && not_params.group_id?)
      items.not_of_projects(not_params.projects).references_project
    else
      items
    end
  end
  ...
  def by_author(items)
    if params.author
      items = items.where(author_id: params.author.id)
  ...
  def by_negated_author(items)
    if not_params.author
      items = items.where.not(author_id: not_params.author.id)
    elsif not_params.no_author?
      items = items.where.not(author_id: nil)
  ...
  # Don't be confused! It's the opposite logic of by_assignee!
  def by_negated_assignee(items)
    if not_params.filter_by_no_assignee?
      items.assigned
    elsif not_params.filter_by_any_assignee?
      items.unassigned
    elsif not_params.assignee
      items.not_assigned_to(not_params.assignee)
  ...
  def by_negated_milestone(items)
    ...
    if not_params.filter_by_no_milestone?
      items.any_milestone
    elsif not_params.filter_by_any_milestone?
      items.left_joins_milestones.where(milestone_id: [-1, nil])
    elsif not_params.filter_by_upcoming_milestone?
      upcoming_ids = Milestone.upcoming_ids(not_params.projects, not_params.related_groups)
      items.where('milestone_id NOT IN (?) OR milestone_id IS NULL', upcoming_ids)
    elsif not_params.filter_by_started_milestone?
      items.where.not(milestone: Milestone.started).or(items.where(milestone_id: nil))
    else
      items.without_milestone(not_params[:milestone_title])
    end
  end
  # rubocop: enable CodeReuse/ActiveRecord
  ...
  def by_negated_release(items)
    ...
    if not_params.filter_by_no_release?
      items.any_release
    elsif not_params.filter_by_any_release?
      items.without_release
    else
      items.without_release(not_params[:release_tag], not_params[:project_id])
    end
  end
  ...
  def by_negated_label(items)
    ...
    items =
      if not_params.filter_by_no_label?
        items.any_label
      elsif not_params.filter_by_any_label?
        items.without_label
      else
        items.without_labels(not_params.label_names)
      end
  ...
  def by_negated_my_reaction_emoji(items)
    ...
    if not_params.filter_by_no_reaction?
      items.awarded(current_user)
    elsif not_params.filter_by_any_reaction?
      items.not_awarded(current_user)
    else
