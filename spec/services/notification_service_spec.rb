...
describe NotificationService, :mailer do
  ...
  around do |example|
    sidekiq_inline = (example.metadata.keys & %i[sidekiq_might_not_need_inline sidekiq_inline]).any?
    if sidekiq_inline
  ...
  describe 'Notes' do
    context 'issue note' do
      ...
      describe '#new_note' do
        context do
          ...
          it "emails the note author if they've opted into notifications about their activity", :sidekiq_might_not_need_inline do
      ...
      describe 'new note on issue in project that belongs to a group' do
        ...
        shared_examples 'new note notifications' do
          it 'sends notifications', :sidekiq_might_not_need_inline do
            notification.new_note(note)
    ...
    context 'confidential issue note', :sidekiq_might_not_need_inline do
    ...
    context 'issue note mention', :sidekiq_might_not_need_inline do
      let(:project) { create(:project, :public) }
      let(:issue) { create(:issue, project: project, assignees: [assignee]) }
      let(:mentioned_issue) { create(:issue, assignees: issue.assignees) }
      let(:author) { create(:user) }
    ...
    context 'project snippet note', :sidekiq_might_not_need_inline do
      let!(:project) { create(:project, :public) }
    ...
    context 'personal snippet note', :sidekiq_might_not_need_inline do
    ...
    context 'commit note', :sidekiq_might_not_need_inline do
    ...
    context "merge request diff note", :sidekiq_might_not_need_inline do
  ...
  describe '#send_new_release_notifications', :sidekiq_might_not_need_inline do
  ...
  describe 'Participating project notification settings have priority over group and global settings if available', :sidekiq_might_not_need_inline do
    let!(:group) { create(:group) }
  ...
  describe 'Issues', :sidekiq_might_not_need_inline do
    let(:group) { create(:group) }
    let(:project) { create(:project, :public, namespace: group) }
  ...
  describe 'Merge Requests', :sidekiq_might_not_need_inline do
    let(:group) { create(:group) }
  ...
  describe 'Projects', :sidekiq_might_not_need_inline do
  ...
  describe 'GroupMember', :sidekiq_might_not_need_inline do
  ...
  describe 'ProjectMember', :sidekiq_might_not_need_inline do
    let(:project) { create(:project) }
    let(:added_user) { create(:user) }
  ...
  context 'guest user in private project', :sidekiq_might_not_need_inline do
  ...
  describe 'Pipelines', :sidekiq_might_not_need_inline do
  ...
  describe 'Pages domains', :sidekiq_might_not_need_inline do
  ...
  context 'Auto DevOps notifications', :sidekiq_might_not_need_inline do
  ...
  describe 'Repository cleanup', :sidekiq_might_not_need_inline do
    let(:user) { create(:user) }
    let(:project) { create(:project) }
  ...
  context 'Remote mirror notifications', :sidekiq_might_not_need_inline do
  ...
  context 'with external authorization service', :sidekiq_might_not_need_inline do
