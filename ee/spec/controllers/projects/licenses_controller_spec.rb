    ...
    context 'with authorized user' do
      ...
      context 'when feature is available' do
        ...
        it 'responds to an HTML request' do
          ...
          expect(assigns(:licenses_app_data)).to be_present
          expect(assigns(:licenses_app_data)[:endpoint]).to eql(controller.helpers.project_licenses_path(project, detected: true, format: :json))
          expect(assigns(:licenses_app_data)[:documentation_path]).to eql(help_page_path('user/application_security/license_compliance/index'))
          expect(assigns(:licenses_app_data)[:empty_state_svg_path]).to eql(controller.helpers.image_path('illustrations/Dependency-list-empty-state.svg'))
        end
