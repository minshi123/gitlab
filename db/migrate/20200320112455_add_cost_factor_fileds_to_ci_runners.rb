...
class AddCostFactorFiledsToCiRunners < ActiveRecord::Migration[6.0]
  ...
  def up
    ...
    add_column_with_default(:ci_runners, :internal_or_private_projects_minutes_cost_factor, :float, default: 1.0)
  end

  def down
    remove_column(:ci_runners, :public_projects_minutes_cost_factor)
    remove_column(:ci_runners, :internal_or_private_projects_minutes_cost_factor)
  end
end
