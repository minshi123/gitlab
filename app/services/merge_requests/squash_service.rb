...
module MergeRequests
  class SquashService < MergeRequests::BaseService
    ...
    SquashForbiddenError = Class.new(RuntimeError)
    ...
    def execute
      ...
      squash_forbidden_check!
      error(s_('MergeRequests|Failed to squash. Should be done manually.'))
    ...
    rescue SquashForbiddenError
      error(s_('MergeRequests|This repository prohibits squashing for merge requests.'))
    end
    ...
    def squash_forbidden_check!
      raise SquashForbiddenError if target_project.squash_option == 'never_squash'
    end
