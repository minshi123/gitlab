...
const fetchReplicableItemsGraphQl = ({ state, dispatch }, direction) => {
  let before = '';
  let after = '';
...
export const fetchReplicableItems = ({ state, dispatch }, direction) => {
  ...
  if (state.useGraphQl) {
    fetchReplicableItemsGraphQl({ state, dispatch }, direction);
  } else if (!state.useGraphQl) {
    fetchReplicableItemsRestful({ state, dispatch });
  }
};
