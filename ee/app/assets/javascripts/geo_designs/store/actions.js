...
import createToast from '~/vue_shared/plugins/global_toast';
...
export const receiveInitiateAllDesignSyncsSuccess = ({ commit, dispatch }, { action }) => {
  createToast(__(`All designs are being scheduled for ${action}`));
...
export const receiveInitiateDesignSyncSuccess = ({ commit, dispatch }, { name, action }) => {
  createToast(__(`${name} is scheduled for ${action}`));
