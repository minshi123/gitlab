...
describe Gitlab::Analytics::CycleAnalytics::Median do
  ...
  it 'returns median duration seconds as float' do
    ...
    Timecop.travel(5.minutes.from_now) do
      merge_request1.metrics.update!(merged_at: Time.now)
    end

    Timecop.travel(10.minutes.from_now) do
      merge_request2.metrics.update!(merged_at: Time.now)
    end

    expect(subject).to be_within(0.1).of(7.5.minutes.seconds)
  end
end
