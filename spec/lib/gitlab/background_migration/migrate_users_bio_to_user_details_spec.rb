...
describe Gitlab::BackgroundMigration::MigrateUsersBioToUserDetails, :migration, schema: 20200323074147 do
  ...
  let!(:user_with_long_bio) { users.create(name: 'user3', email: 'test3@test.com', projects_limit: 1, bio: 'a' * 256) } # 255 is the max

  let!(:user_already_has_details) { users.create(name: 'user4', email: 'test4@test.com', projects_limit: 1, bio: 'my bio') }
  ...
  let!(:user_has_different_details) { users.create(name: 'user5', email: 'test5@test.com', projects_limit: 1, bio: 'different') }
