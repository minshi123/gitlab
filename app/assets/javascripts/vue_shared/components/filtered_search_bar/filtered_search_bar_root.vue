  ...
  data() {
    ...
    if (this.initialSortBy) {
      selectedSortOption = this.sortOptions
        ...
        .pop();
      selectedSortDirection = this.initialSortBy.includes('desc')
        ? SortDirection.descending
        : SortDirection.ascending;
    } else {
      // Fallback to default sort option.
      selectedSortOption = this.sortOptions[0].sortDirection.descending;
    ...
    if (this.recentSearchesStorageKey) {
      dataProps.recentSearchesService = new RecentSearchesService(
        `${this.namespace}-${RecentSearchesStorageKeys[this.recentSearchesStorageKey]}`,
      );
      dataProps.recentSearchesStore = new RecentSearchesStore({
