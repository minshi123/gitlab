...
describe 'gitlab:container_registry namespace rake tasks' do
  ...
  describe 'configure' do
    ...
    context 'when creating a registry client instance' do
      let(:token) { Auth::ContainerRegistryAuthenticationService.access_token([], []) }
      ...
      it 'uses a token with no access permissions' do
        expect(Auth::ContainerRegistryAuthenticationService).to receive(:access_token).with([], []).and_return(token)
        expect(ContainerRegistry::Client).to receive(:new).with(api_url, token: token).and_return(client)
