...
describe Packages::Nuget::MetadataExtractionService do
  ...
  describe '#execute' do
    ...
    context 'with invalid package file id' do
      ...
      it { expect { subject }.to raise_error(ArgumentError, 'invalid package file') }
    end
    ...
    context 'with a 0 byte package file id' do
      before do
        package_file.update!(size: 0.bytes)
      end
    ...
    context 'without the nuspec file' do
      ...
      it { expect { subject }.to raise_error(ArgumentError, 'nuspec file not found') }
    end
    ...
    context 'with a too big nuspec file' do
      ...
      it { expect { subject }.to raise_error(ArgumentError, 'nuspec file too big') }
    end
  end
end
