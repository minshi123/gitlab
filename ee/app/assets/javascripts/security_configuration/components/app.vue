  ...
  computed: {
    devopsMessage() {
      return this.autoDevopsEnabled
        ...
        : __(
            `The configuration status of the table below only applies to the default branch and is based on the %{linkStart}latest pipeline%{linkEnd}. Once you've configured a scan for the default branch, any subsequent feature branch you create will include the scan.`,
          );
    },
    devopsUrl() {
    ...
    fields() {
      return [
        {
          key: 'feature',
          thClass: 'gl-text-gray-900 bg-transparent',
...
<template>
  <article>
    ...
    <gl-table :items="features" :fields="fields" stacked="md">
      <template #head(feature)>
        {{ s__('SecurityConfiguration|Security Control') }}
      </template>
