...
module BulkInsertSafe
  ...
  class_methods do
    ...
    # @param [Symbol]  returns           Pass :ids to return an array with a hash containing a primary
    #                                    key attribute and its value for all inserted records or nil
    ...
    def _bulk_insert_all!(items, on_duplicate:, returns:, unique_by:, validate:, batch_size:, &handle_attributes)
      ...
      # ActiveRecord::InsertAll defaults to the model primary key attributes
      # when returning is set to nil, and omit the underlying RETURNING SQL
      # clause entirely when returning is set to false.
      returning = returns == :ids ? nil : false
