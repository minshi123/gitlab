...
class ApprovalProjectRule < ApplicationRecord
  ...
  def audit_add(model)
    return unless audit_required?

    audit_event_queue.push("Added #{model.class.name} #{model.name} to approval group on #{self.name} rule")
  end

  def audit_remove(model)
    return unless audit_required?

    audit_event_queue.push("Removed #{model.class.name} #{model.name} from approval group on #{self.name} rule")
  end
